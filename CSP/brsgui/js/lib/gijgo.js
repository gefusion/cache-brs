...
gj.dialog.config = {
    base: {
         ...
         * <div id="dialog">Lorem ipsum dolor sit amet, consectetur adipiscing elit...</div>
         * <script>
         *     gj.dialog.messages['fr-fr'].DefaultTitle = 'Titre de la boГ®te de dialogue';
         *     $("#dialog").dialog({
         *         resizable: true,
         *         locale: 'fr-fr',
         *         width: 700
...
gj.dialog.methods = {
    ...
    renderHeader: function ($dialog) {
        ...
        if ($closeButton.length === 0 && data.closeButtonInHeader) {
            $closeButton = $('<button type="button" data-role="close" title="' + gj.dialog.messages[data.locale].Close + '"><span>Г—</span></button>');
...
gj.grid.messages['en-us'] = {
    ...
    Refresh: 'Refresh',
    Of: 'of',
    DisplayingRecords: 'Displaying children',
    RowsPerPage: 'Rows per page:',
    Edit: 'Edit',
    Delete: 'Delete',
    Update: 'Update',
    Cancel: 'Cancel',
    NoRecordsFound: 'No children found.',
    Loading: 'Loading...'
};
...
gj.grid.config = {
    base: {
         ...
         *         alert('The result contains ' + response.children.length + ' children.');
         *         grid.render(response);
         *     };
         *     grid = $('#grid').grid({
         *         dataSource: { url: '', data: {}, success: onSuccessFunc },
         *         columns: [ { field: 'Name' }, { field: 'PlaceOfBirth' } ]
         *     });
         * </script>
        ...
        mapping: {
            /** The name of the object in the server response, that contains array with children, that needs to be display in the grid.
            ...
            /** The name of the object in the server response, that contains the number of all children on the server.
         ...
         * @type string
         * @default "No children found."
         * @example sample <!-- grid -->
         * <table id="grid"></table>
         * <script>
         *     $('#grid').grid({
         *         dataSource: { url: '/Players/Get', data: { name: 'not existing name' } },
         *         notFoundText: 'No children found custom message',
         *         columns: [ { field: 'ID' }, { field: 'Name' }, { field: 'PlaceOfBirth' } ]
         *     });
         * </script>
         ...
         * @additionalinfo If you set primary key, we assume that this number is unique for each children presented in the grid.<br/>
...
gj.grid.events = {
     ...
     *         columns: [ { field: 'ID', width: 56 }, { field: 'Name' }, { field: 'PlaceOfBirth' } ]
     *     });
     *     grid.on('dataBinding', function (e, children) {
     *         alert('dataBinding is fired. ' + children.length + ' children will be loaded in the grid.');
     *     });
     * </script>
     */
    dataBinding: function ($grid, children) {
        $grid.triggerHandler('dataBinding', [children]);
    },
     ...
     * @param {object} e - event data
     * @param {array} children - the list of children
     * @param {number} totalRecords - the number of the all children that can be presented in the grid
     * @example sample <!-- grid -->
     * <table id="grid"></table>
     * <script>
     *     var grid = $('#grid').grid({
     ...
     *         columns: [ { field: 'ID', width: 56 }, { field: 'Name' }, { field: 'PlaceOfBirth' } ]
     *     });
     *     grid.on('dataBound', function (e, children, totalRecords) {
     *         alert('dataBound is fired. ' + children.length + ' children are bound to the grid.');
     *     });
     * </script>
     */
    dataBound: function ($grid, children, totalRecords) {
        $grid.triggerHandler('dataBound', [children, totalRecords]);
    },
     ...
     * @event dataFiltered
     * @param {object} e - event data
     * @param {object} children - The children after the filtering.
     * @example sample <!-- grid -->
     * <table id="grid"></table>
     * <script>
     *     var grid, data = [
     *         { 'ID': 1, 'Name': 'Hristo Stoichkov', 'PlaceOfBirth': 'Plovdiv, Bulgaria', Nationality: 'Bulgaria' },
     *         { 'ID': 2, 'Name': 'Ronaldo LuÃs NazÃ¡rio de Lima', 'PlaceOfBirth': 'Rio de Janeiro, Brazil', Nationality: 'Brazil' },
     ...
     *         { 'ID': 5, 'Name': 'James RodrÃguez', 'PlaceOfBirth': 'CÃºcuta, Colombia', Nationality: 'Colombia' },
     *         { 'ID': 6, 'Name': 'Dimitar Berbatov', 'PlaceOfBirth': 'Blagoevgrad, Bulgaria', Nationality: 'Bulgaria' }
     *     ];
     *     grid = $('#grid').grid({
     *         dataSource: data,
     *         columns: [ { field: 'ID', width: 56 }, { field: 'Name' }, { field: 'PlaceOfBirth' } ],
     *         dataFiltered: function (e, children) {
     *             children.reverse(); // reverse the data
     *             children.splice(3, 2); // remove 2 elements after the 3rd record
     *         }
     *     });
     * </script>
     */
    dataFiltered: function ($grid, children) {
        $grid.triggerHandler('dataFiltered', [children]);
    }
};
...
gj.grid.methods = {
    ...
    autoGenerateColumns: function ($grid, children) {
        var names, value, type, i, data = $grid.data();
        data.columns = [];
        if (children.length > 0) {
            names = Object.getOwnPropertyNames(children[0]);
            for (i = 0; i < names.length; i++) {
                value = children[0][names[i]];
                type = 'text';
                if (value) {
    ...
    loadData: function ($grid) {
        var data, children, i, recLen, rowCount, $tbody, $rows, $row;

        data = $grid.data();
        children = $grid.getAll();
        gj.grid.events.dataBinding($grid, children);
        recLen = children.length;
        gj.grid.methods.stopLoading($grid);

        if (data.autoGenerateColumns) {
            gj.grid.methods.autoGenerateColumns($grid, children);
        }
        ...
        for (i = 0; i < rowCount; i++) {
            if (i < recLen) {
                $row = $rows.eq(i);
                gj.grid.methods.renderRow($grid, $row, children[i], i);
            } else {
        ...
        for (i = rowCount; i < recLen; i++) {
            gj.grid.methods.renderRow($grid, null, children[i], i);
        }
        gj.grid.events.dataBound($grid, children, data.totalRecords);
    },
    ...
    setRecordsData: function ($grid, response) {
        ...
        if ($.isArray(response)) {
            ...
            children = response[data.mapping.dataField];
            totalRecords = response[data.mapping.totalRecordsField];
            if (!totalRecords || isNaN(totalRecords)) {
                totalRecords = 0;
            }
        }
        $grid.data('children', children);
    ...
    getById: function ($grid, id) {
        var result = null, i, primaryKey = $grid.data('primaryKey'), children = $grid.data('children');
        if (primaryKey) {
    ...
    getRowById: function ($grid, id) {
        var children = $grid.getAll(false),
    ...
    filter: function ($grid) {
        var field, column,
            data = $grid.data(),
            children = data.dataSource.slice();

        if (data.params[data.paramNames.sortBy]) {
            column = gj.grid.methods.getColumnInfo($grid, data.params[data.paramNames.sortBy]);
            children.sort(column.sortable.sorter ? column.sortable.sorter(column.direction, column) : gj.grid.methods.createDefaultSorter(column.direction, column.field));
        }

        for (field in data.params) {
            if (data.params[field] && !data.paramNames[field]) {
                column = gj.grid.methods.getColumnInfo($grid, field);
                children = $.grep(children, function (record) {
        ...
        gj.grid.events.dataFiltered($grid, children);
    ...
    updateRow: function ($grid, id, record) {
        ...
        data.children[$row.data('position') - 1] = record;
        if ($.isArray(data.dataSource)) {
...
gj.grid.widget = function ($grid, jsConfig) {
    ...
    /**
     * Return the number of children in the grid. By default return only the children that are visible in the grid.
     * @method
     * @param {boolean} includeAllRecords - include children that are not visible when you are using local dataSource.
     * @return number
     * @example Local.DataSource <!-- bootstrap, grid, grid.pagination -->
     * <button class="btn btn-default" onclick="alert(grid.count())">Count Visible children</button>
     * <button class="btn btn-default" onclick="alert(grid.count(true))">Count All children</button>
     * <br/><br/>
     * <table id="grid"></table>
     * <script>
     *     var data, grid;
     ...
     * <button onclick="alert(grid.count())">Count Visible children</button>
     * <button onclick="alert(grid.count(true))">Count All children</button>
     * <br/><br/>
     * <table id="grid"></table>
     * <script>
     *     var grid = $('#grid').grid({
    ...
    /**
     * Select all children from the grid.
     * @method
     * @return grid
     * @example sample <!-- materialicons, checkbox, grid -->
    ...
    /**
     * Unselect all children from the grid.
     * @method
     * @return void
     * @example sample <!-- materialicons, checkbox, grid -->
     * <button id="btnSelectAll">Select All</button>
    ...
    /**
     * Return an array with all children presented in the grid.
     * @method
     * @param {boolean} includeAllRecords - include children that are not visible when you are using local dataSource.
     * @return number
     * @example Local.DataSource <!-- bootstrap, grid, grid.pagination -->
     * <button onclick="alert(JSON.stringify(grid.getAll()))" class="btn btn-default">Get All Visible children</button>
     * <button onclick="alert(JSON.stringify(grid.getAll(true)))" class="btn btn-default">Get All children</button>
     * <br/><br/>
     * <table id="grid"></table>
     * <script>
     *     var data, grid;
     ...
     *         dataSource: [
     *             { 'ID': 1, 'Name': 'Hristo Stoichkov', 'PlaceOfBirth': 'Plovdiv, Bulgaria' },
     *             { 'ID': 2, 'Name': 'Ronaldo LuÃs NazÃ¡rio de Lima', 'PlaceOfBirth': 'Rio de Janeiro, Brazil' },
     *             { 'ID': 3, 'Name': 'David Platt', 'PlaceOfBirth': 'Chadderton, Lancashire, England' }
     *         ],
     *         columns: [
     *             { field: 'ID', width: 56 },
...
gj.grid.plugins.inlineEditing.config = {
    base: {
        ...
        inlineEditing: {
             ...
             *         { 'ID': 5, 'Name': 'James RodrÃguez', 'PlaceOfBirth': 'CÃºcuta, Colombia' },
             ...
             *         { 'ID': 2, 'Name': 'Ronaldo LuÃs NazÃ¡rio de Lima', 'PlaceOfBirth': 'Rio de Janeiro, Brazil', IsActive: false },
             ...
             *         { 'ID': 5, 'Name': 'James RodrÃguez', 'PlaceOfBirth': 'CÃºcuta, Colombia', IsActive: true },
    ...
    config: {
        base: {
            ...
            paramNames: {
                ...
                /** The name of the parameter that is going to send the maximum number of children per page.
                 * The pager should be enabled in order this parameter to be in use.
            ...
            pager: {
                /** The maximum number of children that can be show by page.
                 * @alias pager.limit
                 * @type number
                 * @default 10
    ...
    public: {
        getAll: function (includeAllRecords) {
            ...
            if (!includeAllRecords && $.isArray(data.dataSource) && data.params[data.paramNames.limit] && data.params[data.paramNames.page]) {
                ...
                page = parseInt(data.params[data.paramNames.page], 10);
                start = (page - 1) * limit;
                return data.children.slice(start, start + limit);
            } else {
...
gj.grid.plugins.rowReorder = {
    config: {
        base: {
            ...
            /** If set, update the value in the field for all children. Accept only field names of columns.
             * @type string
             * @default undefined
            ...
            return function (e, mousePosition) {
                ...
                $trSource.siblings('tr[data-role="row"]').each(function () {
                    ...
                    if ($trTarget.droppable('isOver', mousePosition)) {
                        ...
                        data.children.splice(targetPosition - 1, 0, data.children.splice(sourcePosition - 1, 1)[0]);
                        ...
                        if (data.orderNumberField) {
                            for (i = 0; i < data.children.length; i++) {
                                data.children[i][data.orderNumberField] = i + 1;
                            }
                            for (i = 0; i < $rows.length; i++) {
                                $row = $($rows[i]);
...
gj.grid.plugins.grouping = {
    ...
    private: {
        ...
        grouping: function ($grid, children) {
            var data = $grid.data();
            children.sort(gj.grid.methods.createDefaultSorter(data.grouping.direction, data.grouping.groupBy));
        },
    ...
    configure: function ($grid) {
        ...
        if (data.grouping && data.grouping.groupBy) {
            ...
            $grid.on('dataFiltered', function (e, children) {
                gj.grid.plugins.grouping.private.grouping($grid, children);
            });
        }
    }
};
...
gj.tree.methods = {
    ...
    render: function ($tree, response) {
        if (response) {
            ...
            $tree.data('children', gj.tree.methods.getRecords($tree, response));
    ...
    loadData: function ($tree) {
        ...
        $root.off().empty();
        for (i = 0; i < children.length; i++) {
            gj.tree.methods.appendNode($tree, $root, children[i], 1);
        }
    ...
    getById: function ($tree, id, children) {
        ...
        for (i = 0; i < children.length; i++) {
            if (id == children[i].id) {
                ...
                result = gj.tree.methods.getById($tree, id, children[i].children);
                if (result) {
                    break;
                }
            }
    ...
    getDataById: function ($tree, id, children) {
        var result = gj.tree.methods.getById($tree, id, children);
        return result ? result.data : result;
    },

    getDataByText: function ($tree, text, children) {
        var i, id,
            result = undefined,
            data = $tree.data();
        for (i = 0; i < children.length; i++) {
            if (text === children[i].data[data.textField]) {
                result = children[i].data;
                break;
            } else if (children[i].children && children[i].children.length) {
                result = gj.tree.methods.getDataByText($tree, text, children[i].children);
                if (result) {
                    break;
                }
            }
    ...
    remove: function ($tree, $node) {
        gj.tree.methods.removeDataById($tree, $node.attr('data-id'), $tree.data('children'));
        $node.remove();    
        return $tree;
    },

    removeDataById: function ($tree, id, children) {
        var i;
        for (i = 0; i < children.length; i++) {
            if (id == children[i].id) {
                children.splice(i, 1);
                ...
                gj.tree.methods.removeDataById($tree, id, children[i].children);
            }
        }
    },
...
gj.tree.widget = function ($element, jsConfig) {
    ...
    self.getDataById = function (id) {
        return methods.getDataById(this, id, this.data('children'));
    };
    ...
    self.getDataByText = function (text) {
        return methods.getDataByText(this, text, this.data('children'));
    };
...
gj.dropdown.methods = {
    ...
    render: function ($dropdown, response) {
        ...
        $dropdown.data('children', response);
        $dropdown.empty();
        $list.empty();
    ...
    getRecordByValue: function ($dropdown, value) {
        ...
        for (i = 0; i < data.children.length; i++) {
            if (data.children[i][data.valueField] === value) {
